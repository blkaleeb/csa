        ...
        <div class="block-content block-content-full">
            <div class="row">
                <div class="col-lg-12 space-y-5">
                    ...
                    action="{{$is_edit?route('penjualan.update',$data->id):'#'}}"
                    method="POST" novalidate>
                        ...
                        <div class="row mb-4">
                            <div class="col-lg-6">
                                ...
                                <select name="customer_id" id="" class="customer form-control select2">
                                    <option value="" disabled @if(!$is_edit) selected @endif>Pilih Konsumen</option>

                                    @foreach($customers as $customer)
                                    <option value="{{$customer->id}}" @if($is_edit && $data->customer_id == $customer->id) selected @endif data-item="{{$customer}}"
                            ...
                            <div class="col-lg-6">
                                <div class="">
                                    ...
                                    <p id="customer_address" style="margin: unset !important"> {{$data->customer->customer_address ?? ""}} </p>
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <input type="text" class="js-flatpickr form-control" id="example-flatpickr-custom"
                                    ...
                                    value="{{\Carbon\Carbon::parse($is_edit?$data->order_date:$orderdate)->format('d-m-Y');}}" data-date-format="d-m-Y">
                            </div>
                            <div class="col-xl-6">
                                ...
                                <input type="text" class="js-flatpickr form-control" id="due_date" name="due_date"
                                    placeholder="Jatuh Tempo" value="{{\Carbon\Carbon::parse($is_edit?$data->due_date:$jatuhtempo)->format('d-m-Y');}}"
                                    data-date-format="d-m-Y">
                            </div>
                            <div class="col-lg-6">
                                <label class="form-label" for="example-ltf-email">Supir</label>
                                <select name="supir" id="" class="form-control select2">
                            ...
                            <div class="col-lg-6">
                                ...
                                <select name="kenek" id="" class="form-control select2">
            ...
            <div class ="table-responsive"><table class="table table-hover table-vcenter js-table-sections ">
                ...
                <tbody class="fs-sm">
                    ...
                    <tr>
                        ...
                        <td>{{ $line->qty }}</td>
                        <td >{{ number_format($line->qty * $line->price_per_satuan_id) }} <input class="subtotalOrder d-none" type="hidden" readonly value="{{$line->qty * $line->price_per_satuan_id}}"></td>
                        <td>
                            <div class="dropdown">
...
<div class="content w-100">
    ...
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <h3 class="block-title">Tambah Penjualan</h3>
        </div>
        <div class="block-content block-content-full">
            <div class="row">
                <div class="col-lg-12 ">
  ...
  if(customerid != ""){
    ...
    .done(function( msg ) {
        ...
        $.each(msg,function(k,v){
            ...
            var tanggalorder = `<td>`+new Date(v.order_date).toLocaleDateString()+`</td>`
            var totalfaktur = `<td>`+number_format(v.total_sales, 0, ",", ".")+`</td>`
            var totalbayar = `<td>`+number_format(v.total_paid, 0, ",", ".")+`</td>`
            var retur = `<td>`+number_format(v.retur, 0, ",", ".")+`</td>`
            var sisabayar = `<td>`+number_format(v.payment_remain, 0, ",", ".")+`</td>`
  ...
  if(stockid != "" && customerid != ""){
    ...
    .done(function( msg ) {
        ...
        $.each(msg,function(k,v){
            ...
            var qty = `<td>`+v.qty+`</td>`
            var hargasatuan = `<td>`+number_format(v.price_per_satuan_id, 0, ",", ".")+`</td>`
