    ...
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <div class="float-start">
                <h3 class="block-title">Filter {{$date_start->format('d F Y') . "  -  ". $date_end->format('d F Y')}}</h3>
            </div>
        </div>
        <div class="block-content block-content-full">
            <form autocomplete="off" action="{{route('stok.show',$stock->id)}}">
            <div class="row">
                <div class="col-lg-6">
                    <label class="form-label" for="due_date">Dari</label>
                    <input type="text" class="js-flatpickr form-control" id="example-flatpickr-custom" name="date_start"
                        placeholder="hari-bulan-tahun" value="{{$date_start->format('d F Y')}}"
                        data-date-format="d F Y">
                </div>
                <div class="col-lg-6">
                    <label class="form-label" for="due_date">Sampai</label>
                    <input type="text" class="js-flatpickr form-control" id="example-flatpickr-custom" name="date_end"
                        placeholder="hari-bulan-tahun" value="{{$date_end->format('d F Y')}}" data-date-format="d F Y">
                </div>
            ...
            <div class="tab-pane" id="btabswo-static-detail" role="tabpanel"
                ...
                <div class="row">
                    ...
                    <div class="block block-rounded">
                        ...
                        <div class="block-content block-content-full">
                            <div class ="table-responsive"><table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                                <thead>
                                    <tr>
                            ...
                            <div class ="table-responsive"><table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$item->createdOn->format('d-m-Y');}}</td>
                                        <td>{{$item->header->intnomorsales}}</td>
                                        <td>{{$item->header->customer->name}}</td>
                                        <td>{{$item->qty}}</td>
                                        <td>{{number_format($item->price_per_satuan_id)}}</td>
                                        <td></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            ...
                            <div class ="table-responsive"><table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$item->createdOn->format('d-m-Y');}}</td>
                                        <td>{{$item->header->no_invoice ?? "void"}}</td>
                                        <td>{{$item->qty}}</td>
                            ...
                            <div class ="table-responsive"><table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$item->createdOn->format('d-m-Y');}}</td>
                                        <td>{{$item->header->internal_invoice_no." | ".$item->poline->header->po_no ?? "void"}}</td>
